...
description: Generate high-converting App Store screenshots by analyzing your app's codebase, discovering core benefits, and creating ASO-optimized screenshot images using Antigravity's image generation tools.
user-invocable: true
---
...
- The target audience
- Key app context
- Any reasoning or user preferences noted
...
- **Every simulator screenshot provided** — file path, what it shows, rating, and notes
- **The confirmed pairings** — which benefit maps to which screenshot file
...
- `Prompt`: Enhancement instructions (see prompt templates below)
- `ImagePaths`: Array containing the path to the scaffold (e.g., `["/path/to/scaffold.png"]`)
- `ImageName`: Different name for each version (e.g., `aso_v1`, `aso_v2`, `aso_v3`)
...
- **Target display size**
- **For each generated screenshot**: Benefit headline, subfolder path, chosen version, final path, simulator screenshot used, breakout elements, and status.
