# Antigravity ASO Screenshot Skill

A skill for **Antigravity** that generates high-converting App Store screenshots for your iOS app. It analyzes your codebase, identifies core benefits, and creates professional screenshot images using AI.

## Antigravity Adaptation

This skill has been adapted for **Antigravity**. You can trigger the ASO screenshot generation workflow using the following command:

- `/aso` — Starts the multi-phase screenshot optimization process.

### Configuration

Before you begin, ensure you have set up your `.env` file with your **Gemini API Key**:
...
All progress (benefits, screenshot pairings, and generation status) is saved in the `./.aso_memory/` directory. This allows you to resume your work across different conversations.
...
3. **Generation** — Creates polished App Store screenshots using a two-stage process: deterministic scaffolding (compose.py) + AI enhancement (via Antigravity's `generate_image`)
